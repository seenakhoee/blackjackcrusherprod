...
import { TocDiv, TocLink, TocIcon, Title, TocToggle } from './styles'
...
export default function Toc({ headingSelector, getTitle, getDepth, ...rest }) {
  const { throttleTime = 200, tocTitle = `Instructions & Guides` } = rest
  ...
  return (
    <>
      <TocToggle opener open={open} onClick={() => setOpen(true)} size="1.6em" />
      <TocDiv ref={ref} open={open}>
        <Title>
          <TocIcon />
          {tocTitle || `Instructions & Guides`}
          <TocToggle closer onClick={() => setOpen(false)} />
        </Title>
